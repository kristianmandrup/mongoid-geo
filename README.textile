...
- Created Shape classes to encapsulate the shape/array logic for OuterOperator
- Removed the point.rb file (the goal is to substitute this using GeoPoint of geo_calc)
...
- consider using haversine gem (or extract) for haversine calc
- consider extracting Unit class into geo_calc 
- OuterOperator and TwinOperators are very similar, both have a key and two operators - should be merged somehow?
